<div class="p-4">
    <input type="text" wire:model.live="search" placeholder="Search external participants..." class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline mb-4">
            ...
            @forelse ($externalParticipants as $participant)
                <div class="flex items-center justify-between p-2 border-b last:border-b-0">
                    ...
                    @if (!in_array($participant->id, $selectedParticipants))
                        <button type="button" wire:click="addParticipant({{ $participant->id }})" class="bg-blue-500 hover:bg-blue-700 text-white text-xs py-1 px-2 rounded">Add</button>
                    @else
                        <span class="text-green-500 text-xs">Added</span>
                    @endif
                </div>
            @empty
                <div class="p-2 text-gray-500">No external participants found.</div>
            @endforelse
        </div>
    @endif
    ...
    @else
        <p class="text-gray-500">No external participants selected.</p>
    @endif
</div>